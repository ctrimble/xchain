 ...
 */
public class TestPersistCommand
	extends BaseDatabaseTest
{
	private static final String RESULT = "result";

	private static final String catalogName = "resource://context-class-loader/org/xchain/namespaces/hibernate/test-persist-command.xchain";
	...
	/**
	 * Test persisting outside of a transaction.
	 */
	@Test public void testPersistCommand()
	throws Exception
	{
		...
		Command command = catalog.getCommand("test-persist-command");

                try {
		  command.execute(context);
                  fail("Persisting outside of a transaction should throw an exception.");
                }
                catch( Exception e ) {
                  // this should happen.
                }
	...
	/**
	 * Test persisting inside of transaction.
	 */
	...
	throws Exception
	{
		...
		Command command = catalog.getCommand("test-persist-command-transaction");
		command.execute(context);
	...
	/**
	 * Test persisting while inside a transaction but before an exception is thrown.
	 */
	@Test public void testPersistCommandRollback()
	throws Exception
	{
		Person newPerson = new Person();
		...
		Command command = catalog.getCommand("test-persist-command-rollback");
		try {
			command.execute(context);
