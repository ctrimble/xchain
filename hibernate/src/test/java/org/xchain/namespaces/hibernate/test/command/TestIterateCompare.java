 ...
 */
@Element(localName="iterate-compare")
public abstract class TestIterateCompare
  implements Command
{
	...
	public abstract Iterator getText( JXPathContext context );
	public abstract boolean hasTest();			
		...
		throws Exception
	{
		...
		Iterator<Person> testIterator = getText(context);
		
		boolean equal = true;
		
		while (sourceIterator.hasNext() && testIterator.hasNext() && equal) {
			equal = sourceIterator.next().getName().equalsIgnoreCase(testIterator.next().getName());
		}
		
		context.setValue(getResult(context), equal);
